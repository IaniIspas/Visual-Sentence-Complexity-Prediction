...
class VisualSentenceComplexity:
    ...
    def train_knn(self, feature_matrix, target):
        # Train the KNN model
        self.knn_model = KNeighborsRegressor(**self.knn_params)
        self.knn_model.fit(feature_matrix, target)
    ...
    def execute_pipeline(self):
        ...
        # Train KNN
        self.train_knn(X_train_scaled, y_train)

        # Evaluate on validation set
        validation_predictions = self.knn_model.predict(X_validation_scaled)
        ...
        test_predictions = self.knn_model.predict(X_test_scaled)
...
pipeline = VisualSentenceComplexity(
    ...
    word2vec_params=WORD2VEC_PARAMETERS,
    knn_params=KNN_PARAMETERS,
